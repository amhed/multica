...
func (h *Handler) ListArchivedInboxPage(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.enrichInboxAncestors(r.Context(), filters.WorkspaceID, resp.Items); err != nil {
		writeError(w, http.StatusInternalServerError, "failed to load inbox ancestors")
		return
	}
	if resp.HasMore {
