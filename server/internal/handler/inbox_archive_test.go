...
func TestArchivedInboxPageBeyond200(t *testing.T) {
	...
	parentID := dbfx.Issue(t, "Archive parent", testutil.Cols{"workspace_id": ws})
	// Equal timestamps exercise the ID tie-breaker at every page boundary.
	created := time.Now().UTC().Truncate(time.Microsecond)
	for range 205 {
		issue := dbfx.Issue(t, "Archived", testutil.Cols{"workspace_id": ws, "parent_issue_id": parentID})
	...
	for pageNumber := 0; ; pageNumber++ {
		...
		for _, item := range page.Items {
			...
			if len(item.IssueAncestors) != 1 || item.IssueAncestors[0].ID != parentID {
				t.Fatalf("archive page lost parent context: %+v", item.IssueAncestors)
			}
...
func TestArchivedInboxFiltersAndLookupUseLatestGroup(t *testing.T) {
	...
	dbfx.Member(t, ws, testUserID, "owner")
	parentID := dbfx.Issue(t, "Lookup parent", testutil.Cols{"workspace_id": ws})
	issue := dbfx.Issue(t, "Older matching group", testutil.Cols{"workspace_id": ws, "status": "in_review", "priority": "high", "parent_issue_id": parentID})
	...
	lookup := get("group_id=" + issue)
	if len(lookup.Items) != 1 {
		t.Fatal("deep link outside first page did not resolve")
	}
	if len(lookup.Items[0].IssueAncestors) != 1 || lookup.Items[0].IssueAncestors[0].ID != parentID {
		t.Fatal("deep link outside first page lost parent context")
	}
