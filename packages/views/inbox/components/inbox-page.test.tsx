...
describe("InboxPage", () => {
  ...
  it("opens a deep-linked archive group outside the loaded pages with its comment anchor", () => {
    ...
    layout.width = DESKTOP;
    useInboxViewStore.setState({ collapsedByView: { "workspace-1:archived": new Set(["workspace-1:issue:parent-issue"]) } });
    ...
    expect(screen.getAllByTestId("row").map(row => row.textContent)).toContain("older");
    act(() => rowActions?.onAction("older"));
    expect(unarchiveMutate).toHaveBeenCalledWith("older", expect.anything());
    expect(replace).toHaveBeenLastCalledWith("/acme/inbox?view=archived&issue=issue-1");
  });
