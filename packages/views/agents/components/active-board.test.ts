...
describe("taskSummary", () => {
  it("uses the trigger summary when there is no generated headline", () => {
    expect(taskSummary(task({ trigger_summary: "cmt" }))).toEqual({
      source: "trigger_summary",
      text: "cmt",
    });
    expect(
      taskSummary(task({ trigger_summary: "  ", kind: "comment" })),
    ).toEqual({
      source: "kind",
      kind: "comment",
    });
  ...
  it("falls back to the task kind when no text is available", () => {
    expect(taskSummary(task({ kind: "direct" }))).toEqual({
      source: "kind",
      kind: "direct",
    });
...
describe("taskSummary with pstack_summary", () => {
  it("prefers the generated headline over the trigger", () => {
    expect(
      taskSummary(
        task({ pstack_summary: "Adds a flag.", trigger_summary: "trigger" }),
      ),
    ).toEqual({
      source: "pstack_summary",
      text: "Adds a flag.",
    });
  });
  ...
  it("ignores a blank or missing headline", () => {
    expect(
      taskSummary(task({ pstack_summary: "   ", trigger_summary: "note" })),
    ).toEqual({
      source: "trigger_summary",
      text: "note",
    });
    expect(
      taskSummary(task({ pstack_summary: null, trigger_summary: "trigger" })),
    ).toEqual({
      source: "trigger_summary",
      text: "trigger",
    });
...
function textStep(content: string, seq = 2): TraceStep {
  return {
    kind: "text",
    seq,
    item: { seq, type: "text", content } as never,
    startedAt: "2026-09-03T10:00:00Z",
  };
}
...
describe("describeStep", () => {
  it("maps file tools to edited/read with the path as object", () => {
    expect(
      describeStep(
        callStep("Edit", { file_path: "server/internal/cli/issue_list.go" }),
      ),
    ).toEqual({
      verb: "edited",
    ...
    expect(describeStep(callStep("Write", { file_path: "a.ts" })).verb).toBe(
      "edited",
    );
    expect(describeStep(callStep("Read", { file_path: "a.ts" })).verb).toBe(
      "read",
    );
  });
  ...
  it("treats any call with a command string as ran", () => {
    expect(
      describeStep(callStep("Bash", { command: "go test ./internal/cli" })),
    ).toEqual({
      verb: "ran",
      object: "go test ./internal/cli",
      tone: "normal",
    });
    expect(describeStep(callStep("shell", { command: "pnpm test" })).verb).toBe(
      "ran",
    );
  });

  it("maps search tools to searched", () => {
    expect(describeStep(callStep("Grep", { pattern: "pstack" })).verb).toBe(
      "searched",
    );
    expect(
      describeStep(callStep("WebSearch", { query: "sqlc narg" })).object,
    ).toBe("sqlc narg");
  });

  it("falls back to used with the tool name for unknown tools", () => {
    expect(
      describeStep(callStep("mcp__linear__get_issue", { id: "MUL-1" })),
    ).toEqual({
      verb: "used",
  ...
  it("renders text as said and errors with the error tone", () => {
    ...
    const err: TraceStep = {
      kind: "error",
      seq: 3,
      item: { seq: 3, type: "error", content: "boom" } as never,
    };
    expect(describeStep(err)).toEqual({
      verb: "errored",
      object: "boom",
      tone: "error",
    });
...
describe("isWaitingForInput", () => {
  ...
  it("is true for a recently completed task whose last text asks a question", () => {
    expect(
      isWaitingForInput(
        ...
        [
          callStep("Edit", {}),
          textStep("Re-check on every run, or only at creation?"),
        ],
        now,
      ),
    ).toBe(true);
  ...
  it("is false when the last text is not a question", () => {
    expect(isWaitingForInput(done, [textStep("Done, PR is open.")], now)).toBe(
      false,
    );
  });
  ...
  it("is false past the one hour cut-off", () => {
    const old = task({
      status: "completed",
      completed_at: new Date(now - RECENT_TERMINAL_MS - 1000).toISOString(),
    });
  ...
  it("is false for running or failed tasks and when there is no text", () => {
    ...
    expect(
      isWaitingForInput(
        task({ status: "failed", completed_at: "2026-09-03T11:59:00Z" }),
        [textStep("Which one?")],
        now,
      ),
    ).toBe(false);
...
describe("selectBoardTasks and sortBoardCards", () => {
  ...
  it("keeps active tasks and recently completed ones, drops old terminal tasks", () => {
    const snapshot = [
      ...
      task({
        id: "recent",
        status: "completed",
        completed_at: "2026-09-03T11:50:00Z",
      }),
      task({
        id: "old",
        status: "completed",
      ...
      task({
        id: "cancelled",
        status: "cancelled",
        completed_at: "2026-09-03T11:59:00Z",
      }),
    ];
    expect(selectBoardTasks(snapshot, now).map((t) => t.id)).toEqual([
      "run",
      "recent",
    ]);
  });
  ...
  it("orders waiting, then running by activity, then queued; drops non-waiting terminal tasks", () => {
    const cards = [
      ...
      {
        task: task({ id: "r-old", status: "running" }),
        waiting: false,
        lastActivityAt: "2026-09-03T11:10:00Z",
      },
      {
        task: task({
          id: "done",
          status: "completed",
          completed_at: "2026-09-03T11:55:00Z",
        }),
        waiting: false,
        lastActivityAt: null,
      },
      ...
      {
        task: task({ id: "r-new", status: "running" }),
        waiting: false,
        lastActivityAt: "2026-09-03T11:59:00Z",
      },
    ];
    expect(sortBoardCards(cards).map((c) => c.task.id)).toEqual([
      "w",
      "r-new",
      "r-old",
      "q",
