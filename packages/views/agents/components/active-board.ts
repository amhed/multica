...
export function activeCounts(tasks: readonly AgentTask[]): {
  running: number;
  waiting: number;
} {
  let running = 0;
...
export function describeStep(step: TraceStep): StepDescription {
  if (step.kind === "call") {
    ...
    if (typeof input?.command === "string")
      return { verb: "ran", object: summary, tone: "normal" };
    ...
    const filePath =
      typeof input?.file_path === "string" ? input.file_path : summary;
    if (EDIT_TOOLS.has(step.tool))
      return { verb: "edited", object: filePath, tone: "normal" };
    if (READ_TOOLS.has(step.tool))
      return { verb: "read", object: filePath, tone: "normal" };
    if (SEARCH_TOOLS.has(step.tool))
      return { verb: "searched", object: summary, tone: "normal" };
    return {
      verb: "used",
      object: [step.tool, summary].filter(Boolean).join(" "),
      tone: "normal",
    };
  }
  const text = plainSummary(step.item.content ?? "");
  if (step.kind === "error")
    return { verb: "errored", object: text, tone: "error" };
...
export function isWaitingForInput(
  ...
  if (
    ...
    !completedWithin(task, RECENT_TERMINAL_MS, now)
  )
    return false;
...
export function selectBoardTasks(
  ...
  return snapshot.filter(
    (t) =>
      ...
      (t.status === "completed" && completedWithin(t, RECENT_TERMINAL_MS, now)),
  );
}
...
export function sortBoardCards(cards: readonly BoardCard[]): BoardCard[] {
  const startOf = (t: AgentTask) =>
    t.started_at ?? t.dispatched_at ?? t.created_at;
  return cards
    .filter((c) => c.waiting || ACTIVE_STATUSES.has(c.task.status))
    .sort((a, b) => {
      if (a.waiting !== b.waiting) return a.waiting ? -1 : 1;
      const rank =
        (STATUS_RANK[a.task.status] ?? 9) - (STATUS_RANK[b.task.status] ?? 9);
