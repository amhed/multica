...
- When every PR linked to the issue is merged and one of them closes it with a keyword such as `Closes MUL-123`, the issue becomes `done` — unless PR auto-complete is off for the workspace or the issue. See [GitHub integration](/github-integration).
</Callout>
...
- **Archiving requires an empty status.** Move every issue using it to another status first, including sub-issues and completed or cancelled issues, then retry **Archive**. Archiving retires the status without deleting it or moving issues automatically. It disappears from status choices and default board, list, and swimlane groups. For a status archived by an older release that still has issues, use **Settings → Issue Statuses → Show archived → View issues** to find and move them; this view does not change your saved filters.
