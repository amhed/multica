...
| `MULTICA_LLM_SUMMARY_MODEL` | empty | Model for the one-line agent headlines on the Active board; falls back to `MULTICA_LLM_DEFAULT_MODEL` when empty |
| `MULTICA_LLM_MAX_RETRIES` | `2` | Retry ceiling per call; `0` disables retries, `1`–`5` cap it at N |
| `MULTICA_LLM_DISABLE_THINKING` | `false` | Add `chat_template_kwargs: {"enable_thinking": false}` to every request, for gateways that honor the field |
...
- **Active board headlines** (the one-line summary shown for a running task) — the issue identifier and title, the first 1500 characters of the issue description, the task's trigger summary (the comment or autopilot text that woke the agent), and the handoff note.

When both the API key and the base URL are empty, this layer is disabled and makes no upstream request at all — none of the features above send anything. That is the supported configuration when your policy does not allow this layer to send chat content off the deployment: chat sessions keep the title the client derives from the first message, the follow-up question buttons do not appear, the Active board keeps showing the trigger text instead of a generated headline, and everything else is unaffected.
