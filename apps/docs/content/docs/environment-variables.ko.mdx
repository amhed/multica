...
| `MULTICA_LLM_SUMMARY_MODEL` | 비어 있음 | 진행 중 보드의 에이전트 한 줄 헤드라인에 사용하는 모델. 비어 있으면 `MULTICA_LLM_DEFAULT_MODEL`로 대체 |
| `MULTICA_LLM_MAX_RETRIES` | `2` | 호출당 재시도 상한. `0`은 재시도 비활성화, `1`–`5`는 최대 N회 |
| `MULTICA_LLM_DISABLE_THINKING` | `false` | 모든 요청에 `chat_template_kwargs: {"enable_thinking": false}`를 첨부. 해당 필드를 해석하는 gateway용 |
...
`MULTICA_LLM_DISABLE_THINKING`은 모델이 추론(thinking) 패스를 실행해 어시스트 호출의 지연 예산을 잠식하는 gateway를 위한 설정입니다. `true`(또는 `1`)로 설정하면 서버가 모든 요청 본문에 `chat_template_kwargs: {"enable_thinking": false}`를 첨부하며, 이 필드를 전달하는 gateway(vLLM/sglang 계열 배포, 일부 GLM/Qwen 계열 LiteLLM 라우트)는 thinking 패스를 건너뜁니다. 표준 OpenAI endpoint는 알 수 없는 본문 필드를 거부하므로, 업스트림이 이를 받아들이는 경우에만 활성화하세요. GPT-5.6 계열 모델은 후속 질문 요청에서 서버가 이미 `reasoning_effort: none`을 붙입니다. 채팅 자동 제목 생성은 추론 필드를 보내지 않습니다. 이 설정이 그것에 영향을 주는 것은 구성된 업스트림이 `chat_template_kwargs`를 받아들이는 경우뿐입니다. 허용되는 값은 `true`/`false`와 `1`/`0`(대소문자 구분 없음)이며, 그 외 값은 시작에 실패합니다. 시작 로그에 재시도 정책과 함께 유효 상태가 기록됩니다.

이 레이어를 사용하는 기능은 세 가지이며, 모두 설정한 endpoint로 채팅 내용을 전송합니다.
...
- **진행 중 보드 헤드라인**(실행 중인 태스크의 한 줄 요약) — 해당 태스크에 연결된 이슈의 번호와 제목, 이슈 설명의 앞 1500자, 이 실행을 촉발한 트리거 요약(에이전트를 깨운 댓글 또는 자율 실행 텍스트), 그리고 핸드오프 노트를 전송합니다.

API key와 base URL이 모두 비어 있으면 이 레이어가 꺼지고 업스트림 요청을 전혀 보내지 않습니다. 위 세 기능 모두 아무것도 전송하지 않습니다. 이 레이어가 채팅 내용을 배포 환경 밖으로 보내면 안 되는 정책이라면 이것이 지원되는 구성입니다. 세션은 클라이언트가 첫 메시지에서 만든 제목을 그대로 사용하고, 후속 질문 버튼은 표시되지 않으며, 진행 중 보드는 생성된 헤드라인 대신 트리거 요약을 계속 표시하고, 나머지 기능은 영향을 받지 않습니다.
