...
| `MULTICA_LLM_SUMMARY_MODEL` | 空 | 稼働中ボードのエージェント見出し（1 行要約）に使用するモデル。空の場合は `MULTICA_LLM_DEFAULT_MODEL` にフォールバック |
...
このレイヤーを使う機能は 3 つあり、いずれも設定した endpoint にチャットの内容を送信します。
...
- **稼働中ボードの見出し**（実行中タスクの 1 行要約） — タスクの紐づくイシューの番号とタイトル、イシューの説明の先頭 1500 文字、この実行のきっかけとなったトリガー要約（エージェントを起動したコメントまたは自動運転のテキスト）、そしてハンドオフノートを送信します。

API key と base URL の両方が空の場合、このレイヤーは無効になり、上流へのリクエストを一切行いません。上記 3 つの機能はいずれも何も送信しなくなります。このレイヤーからチャットの内容をデプロイ外へ出せないポリシーでは、これがサポートされた構成です。セッションはクライアントが最初のメッセージから生成したタイトルをそのまま使い、フォローアップ質問のボタンは表示されず、稼働中ボードは生成された見出しの代わりにトリガー要約を表示し続け、他の機能への影響はありません。
