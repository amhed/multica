...
| `MULTICA_LLM_SUMMARY_MODEL` | 空 | "进行中"看板智能体一句话标题使用的模型；为空时回退到 `MULTICA_LLM_DEFAULT_MODEL` |
| `MULTICA_LLM_MAX_RETRIES` | `2` | 单次调用的重试上限；`0` 表示禁用重试，`1`–`5` 表示最多重试 N 次 |
| `MULTICA_LLM_DISABLE_THINKING` | `false` | 在每个请求中附带 `chat_template_kwargs: {"enable_thinking": false}`，供支持该字段的网关关闭模型思考 |
...
- **"进行中"看板一句话标题**（智能体当前这次执行的一句话摘要）—— 发送对应任务的编号和标题、任务描述的前 1500 个字符、触发这次执行的说明（唤醒智能体的评论或自动驾驶文本），以及交接备注。

API key 与 base URL 都为空时，这一层关闭，不会发出任何上游请求——上面三个功能都不再发送内容。如果你的政策不允许这一层把聊天内容发到部署之外，这就是受支持的配置方式：对话继续使用客户端根据第一条消息生成的标题，后续提问按钮不再出现，"进行中"看板继续显示触发说明而不是生成的一句话标题，其余功能不受影响。
