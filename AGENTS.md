...
`.mcp.json` registers codebase-memory-mcp for this repository. Prefer its knowledge graph for code discovery: `search_graph`, `trace_path`, `get_code_snippet`, then `query_graph` / `get_architecture`. Index an unindexed checkout with `index_repository` and refresh after large merges. Use text search for strings, configs, non-code files, or insufficient graph results.
